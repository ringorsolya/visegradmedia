...
subtitle: "Optimized for Hungarian Language Texts"
author: "Your Name"
date: "`r Sys.Date()`"
output: 
  html_document:
...
This R Markdown document provides a comprehensive workflow for preprocessing Hungarian text data for topic modeling. The script includes multiple approaches using different R packages and provides quality checks to ensure the data is ready for topic modeling algorithms.
...
library(hunspell)     # Hungarian spell checking
...
Define comprehensive text cleaning functions optimized for Hungarian texts.
...
clean_text <- function(text) {
  ...
  # Remove special characters, keep only Hungarian letters and spaces
  text <- gsub("[^áéíóöőúüűa-z\\s]", " ", text)
...
# Hungarian stemming function
hungarian_stemmer <- function(text) {
  # Basic Hungarian suffixes to remove
  endings <- c("nak", "nek", "ban", "ben", "ról", "ről", "tól", "től", 
               "hoz", "hez", "höz", "val", "vel", "ért", "ként", "kor",
               "ság", "ség", "ás", "és", "ós", "ős", "ék", "ok", "ek", "ök")
...
# Hungarian stopwords (expandable list)
hungarian_stopwords <- c(
  "a", "az", "egy", "és", "de", "hogy", "is", "mint", "vagy", "ha", "ezt", 
  "azt", "ez", "az", "el", "fel", "le", "be", "ki", "át", "meg", "volt", 
  "van", "lesz", "lehet", "kell", "által", "után", "alatt", "között", "mellett",
  "nélkül", "ellen", "szerint", "miatt", "során", "végett", "helyett", "előtt",
  "után", "körül", "közül", "közöl", "felől", "felé", "erre", "arra", "ide",
  "oda", "itt", "ott", "amikor", "ahol", "ahogy", "amely", "aki", "ami",
  "igen", "nem", "csak", "már", "még", "akkor", "most", "itt", "ott"
)

cat("Defined", length(hungarian_stopwords), "Hungarian stopwords\n")
```
...
# Remove stopwords
corpus <- tm_map(corpus, removeWords, hungarian_stopwords)
corpus <- tm_map(corpus, removeWords, stopwords("hungarian")) # if available

# Apply stemming
corpus <- tm_map(corpus, content_transformer(hungarian_stemmer))
...
# Remove stopwords
tokens <- tokens_remove(tokens, hungarian_stopwords)
...
tidy_text <- df %>%
  ...
  anti_join(data.frame(word = hungarian_stopwords), by = "word") %>%
...
saveRDS(dtm_filtered, "dtm_for_topic_modeling.rds")

# Save cleaned texts
write.csv(df, "cleaned_texts.csv", row.names = FALSE, fileEncoding = "UTF-8")

# Save quanteda DFM
saveRDS(dfm_trimmed, "dfm_for_topic_modeling.rds")

cat("Files saved:\n")
cat("- dtm_for_topic_modeling.rds: Document-Term Matrix (tm package)\n")
cat("- dfm_for_topic_modeling.rds: Document-Feature Matrix (quanteda)\n")
cat("- cleaned_texts.csv: Original and cleaned texts\n")
```
...
- Customize Hungarian stopwords list if needed
